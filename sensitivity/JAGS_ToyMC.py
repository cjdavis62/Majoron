...
def write_batch_script(spectral_index, livetime_dir, queue, iterations):
    ...
    qsub_file.write("cp b2g-MC_data_${taskID}.txt /nfs/cuore1/scratch/cjdavis/MajoronSensitivity/n%s/%s/b2g_files/." %(spectral_index, livetime_dir))
...
def create_output_dir(spectral_index, livetime_name):
    # check if directory already exists
    if not (os.path.isdir("/nfs/cuore1/scratch/cjdavis/MajoronSensitivity/n%s/%s/b2g_files/" %(spectral_index, livetime_name))):
                # use makedirs because it makes directories in a recursive and safe way (no craziness from \rm -rf *\
                os.makedirs("/nfs/cuore1/scratch/cjdavis/MajoronSensitivity/n%s/%s/b2g_files/" %(spectral_index, livetime_name))

# Start the program
args = get_args()

write_batch_script(args.index, args.livetime_dir, args.queue, args.iterations)
create_output_dir(args.index, args.livetime_dir)
