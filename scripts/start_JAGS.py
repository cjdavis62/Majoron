...
def get_arguments():
    ...
    parser.add_argument("-n", "--spectral_index", type=float, help="the spectral index, i.e. 1, 2, 3, 7")
    parser.add_argument("-D", "--data_name", type=str, help="the name of the data file")
    parser.add_argument("-p", "--setup_path", type=float, help="The path to the setup files, e.g. '/home/cjdavis/JAGS_setup'")
    parser.add_argument("-M", "--multiplicities", type=str, help="The multiplicities to consider, viz. M1, M2, and/or M2sum. Input as '-M1 -M2 -M2sum'")
    ...
    if not(os.path.isfile("{setup_path}/n{spectral_index}/setup_{systematic}_{spectral_index}".format(setup_path=args.setup_path, spectral_index=args.spectral_index, systematic=args.systematic))):
        print("Error: setup file not found at path: {setup_path}/n{spectral_index}/setup_{systematic}_{spectral_index}".format(setup_path=args.setup_path, spectral_index=args.spectral_index, systematic=args.systematic))
        sys.exit(1)
    return args

def PrepareData_script_gen(data_name, multiplicies, systematic):
    ...
    while (len(multiplicities) != 0):
        Multiplicities_output = Multiplicities_output + str(args.multiplicities.pop(0)) + "_" + systematic + " "
...
def main():
    ...
    os.system("source {setup_path}/n{spectral_index}/setup_{systematic}_{spectral_index}".format(setup_path=args.setup_path, spectral_index=args.spectral_index, systematic=args.systematic))
    ...
    prepareDataScript = PrepareData_script_gen(args.data_name, args.multiplicies, args.systematic)
